...
weights.true = rbind(W.xl, t(W.lt))


W = W.xl %*% W.lt + matrix(rnorm(n*p, 0, 0.5), nrow = n)


guide.list = get_guide(W, K = k, ica_runs = 1, verbose=F) ## no need for ICASSO in simple optimization settings
...
## Showcasing how GUIDE's approach for estimating a number of components becomes incosistent 
...
for (ii in 1:n.reps) {
  ...
  K.est.1 = get.nlatents(W, starting.K = K.1, validation.reps = 30, alg.typ = "deflation")$K
...
for (ii in 1:n.reps) {
  ...
  K.est.2 = get.nlatents(W, starting.K = K.2, validation.reps = 30, alg.typ = "deflation")$K
...
## GUIDE is mostly correct with the bigger structure, but sometimes is wrong despite the clear present of 10 non-random components
...
## Showing how GUIDE's estimation of the number of components breaks down with increasing levels of component dependence
...
for (jj in 1:n.reps) {
  ...
  for (ii in 1:length(dependence_probs)){
    ...
    nlat.output = get.nlatents(W, starting.K = K, validation.reps = 30, alg.typ = "deflation")$K
